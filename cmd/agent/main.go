...
*Chú ý 1: Tiền điện, tiền nước phải thuộc "Hóa đơn & Tiện ích", tuyệt đối không phân vào "Ăn uống". Nếu là mua vé xem phim thì là "Giải trí".
*Chú ý 2 (ĐA TIỀN TỆ): Nếu phát hiện giao dịch sử dụng ngoại tệ (USD, Baht, Yên, Won, Euro...), BẮT BUỘC tự động ước lượng tỷ giá hiện tại và quy đổi sang VNĐ. Trường "amount" CHỈ LƯU SỐ TIỀN VNĐ. Ở "description", bắt buộc ghi chú số tiền gốc (VD: "Ăn tối (500 Baht)").
Nhiệm vụ: Phân tích và CHỈ trả về một MẢNG (ARRAY) JSON có các key: 
- "date" (YYYY-MM-DD)
- "type" ("Thu" hoặc "Chi")
- "amount" (số nguyên dương, theo VNĐ)
